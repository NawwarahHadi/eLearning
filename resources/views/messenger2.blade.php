 ...
 */
    ...
    document.addEventListener('DOMContentLoaded', function () {
        if (typeof window.Echo !== 'undefined') {
            ...
            console.error('Echo is not defined — check echo.js / app.js is loaded');
        }
    });
</script> --}}
...
<script>
    ...
    function handleReschedule(messageId, actionType) {
        const actionContainer = document.getElementById(`actions-${messageId}`);
        if (!confirm(`Are you sure you want to ${actionType} this class reschedule request?`)) return;

        actionContainer.innerHTML = `<small class="text-muted font-italic">Processing state update...</small>`;

        axios.post(`/chat/reschedule/${actionType}`, { message_id: messageId })
        .then(response => {
            if (actionType === 'approve') {
                actionContainer.innerHTML = `<span class="badge bg-success p-2">✅ Approved - Zoom Room Spawned</span>`;
            } else {
    ...
    function appendMessage(text, type, messageId = null) {
        ...
        if (text.includes('[RESCHEDULE_REQUEST]')) {
            const parsed = JSON.parse(text.replace('[RESCHEDULE_REQUEST]', '')) || {};

            // Buttons only for a tutor receiving the request live
            const tutorButtons = (!isSender && window.currentUserRole === 'tutor' && messageId)
            ...
            const parsed = JSON.parse(text.replace('[RESCHEDULE_RESOLVED_APPROVED]', '')) || {};
            innerContent = `
                <div class="text-muted">
                    <h6 class="font-weight-bold text-success mb-1">✅ Reschedule Approved</h6>
                    <hr class="my-1">
